    ...
    T: MetricType,
{
    ...
    fn dynamic_label<'a>(&mut self, glean: &Glean, label: &'a str) -> &'a str {
        if self.seen_labels.is_empty() && self.meta.lifetime != Lifetime::Application {
            // Fetch all labels that are already stored by iterating through existing data.

            let prefix = format!("{}/", self.meta.identifier());
            ...
            let lifetime = self.meta.lifetime;
            for store in &self.meta.send_in_pings {
                glean
                    .storage()
